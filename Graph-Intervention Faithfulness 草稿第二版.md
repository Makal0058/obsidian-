...
首先需要进一步说明为什么独立重试零基线 $1-(1-p)^K$ 可以看作一种理想化参照。本文中 LLM 需要处理受控定长多跳图推理任务 $G=(V,E)$，每处理一个基础样本可以看作 LLM 抽样回答一次问题，则每次 retry 可以看作 LLM 重新抽样回答问题。然而，与人类不同的是，人类处理错误时会积累经验，在 retry 时更新策略；而通过 API 调用的 LLM 无法自发更新参数，且上下文记忆仅限于本轮对话反馈的信息，导致模型的图追踪能力不会随 retry 自发增强，每次 LLM 在参数层面与第一次抽样回答几乎完全等价（LLM 重试在参数层面不发生学习或更新；不同尝试之间的差异主要来自 verifier feedback 改变了上下文，以及 temperature=0.3 带来的采样随机性）。这使得”独立重采样“可以作为一个合理的理想化乐观参照，若所有样本难度相同且共享同一个单次通过率 $p$、每次抽样独立，则 pass@K 应满足 $1-(1-p)^K$。然而，真实任务的样本难度通常本身并不均匀，且 verifier 的提示可能使同一样本难度在时间上具有异质性，因此不同样本可能具有不同的单次通过率 $p_i$，一个具有鲁棒性的思路是使用整体 pass@1 估计出的统一 $p$ 计算 $1-(1-p)^K$。若实测 pass@K 明显低于该基线，则说明失败并非均匀随机分布，而可能由于样本难度异质性或稳定结构瓶颈。



观察到的 pass@K 与独立重试零基线偏离不来自“学习”而只能来自样本难度的异质性——存在一部分对该模型而言在"参数能力外"的样本，无论抽多少次都难以通过。





这也把 pass@K 相对独立基线的偏离转化为对样本难度结构的诊断，若 pass@K 贴合基线（Qwen），说明在该模型视角下样本难度近似均匀——大多数样本都处在“低概率但有可能‘的同一区间，retry 类似在同一枚不公平硬币上反复抛掷，每次都有相同的小概率通过，但并不意味着 retry 在修复错误，而是意味着”样本对该模型而言基本均匀地难，多抛几次自然有几次走运通过“；若 pass@K 显著低于基线（DS-Flash），则说明该模型视角下样本难度强烈异质——简单样本被首次抽样吸收完，剩余样本对该模型的当前能力构成稳定瓶颈，每次重试对这些样本仍是低概率，且这个低概率不会因为”看了反馈“而提升，因为 LLM 无法跨轮积累经验。


。进一步的，倘若LLM 的每次 retry 都是在同一模型能力下重新抽样回答问题，且回答失败的问题间没有稳定结构相关，则 LLM 的 pass@K 应近似满足曲线 $1-(1-p)^K$，这一曲线命名为独立重试零基线，作为 LLM verifier-retry 的理想化参照是合理的，
...
这里情形，错题之间没有强结构相关，在这种情况下的重复尝试主要解决偶发失误、容易出错但可恢复的问题；一旦某些样本本身超过了模型当前的状态追踪能力，模型并不会因为重复尝试就自然掌握正确转移。这也说明了独立重试基线为什么是合理的零模型，
...
LLM 的 verifier-retry 与人类反复做题不同：人类会记住上一次错在哪里，并在下一次重做时更新策略；而 API 调用中的模型参数并不会在 retry 之间更新，verifier 反馈也只提供结构错误信息，而不提供正确路径、正确答案或正确下一跳。因此，retry 更接近在固定模型能力下的重新生成，而不是跨轮学习。正因如此，独立重试零基线可以作为一个合理的理想化参照：若每次尝试都以相同单次通过率 $p$ 近似独立成功，则最多尝试 $K$ 次的通过率应为 $1-(1-p)^K$。

但这个零基线还隐含另一个关键假设：所有样本共享同一个成功概率 $p$。实际数据中，样本难度往往是异质的。更准确地说，每个样本都有自己的单次通过概率 $p_i$。若样本难度均匀，则用整体 pass@1 估计出的平均 $p$ 可以较好预测 pass@K；但若样本难度高度异质，例如一部分样本很容易、另一部分样本对模型而言几乎总是失败，那么真实 pass@K 更接近 $\frac{1}{N}\sum_i \left(1-(1-p_i)^K\right)$，而不是 $1-\left(1-\frac{1}{N}\sum_i p_i\right)^K$。由于 $1-(1-p)^K$ 关于 $p$ 是凹函数，样本难度异质性会使真实累计通过率低于用平均 $p$ 计算出的独立重试零基线。因此，实测 pass@K 低于独立基线，并不必然说明参数层面的 retry 不独立；它更直接地说明样本层面的成功概率分布并不均匀，剩余失败中存在一批稳定 hard cases。

这解释了 DeepSeek-V4-Flash 为什么会明显低于独立重试基线。DS-Flash 的 pass@1 较高，说明它在首次尝试中已经解决了大量容易样本；但首次失败后留下的样本并不是同等难度的随机失败，而更可能集中在对该模型而言稳定困难的状态转移上。换言之，容易样本被较早吸收，剩余样本的 $p_i$ 很低，继续 retry 对这些样本仍然像重复面对同一个难题，因此 pass@K 会提前饱和。下一节的 FailureHop 分析将进一步检验这一点：如果这些 hard cases 主要集中在 hop 2 等分叉状态更新位置，那么 DS-Flash 低于独立基线就可以解释为样本难度异质性与结构瓶颈共同作用的结果。

相比之下，Qwen Max 贴近独立重试基线，并不意味着它没有难样本或结构错误，而是说明在 Qwen 的能力视角下，大多数样本都处于相近的低成功率区间。其 pass@1 只有 8.4%，说明模型整体单次通过率很低；但由于样本间成功概率差异没有像 DS-Flash 那样形成明显的“易样本/难样本”分层，整体行为更接近统一低 $p$ 下的重复采样。因此，Qwen 的 pass@K 基本贴合 $1-(1-p)^K$：每次 retry 都像在同一枚低成功率硬币上再次采样，多试几次自然会带来累计提升，但这并不表示 verifier-retry 在系统性修复结构错误。




以及为什么不同模型会相对该基线呈现不同偏离。首先，
...
Qwen 单次通过率较低且失败近独立，其 pass@K 基本贴近独立重试基线，重复尝试的提升主要来自解决较容易问题，较少触及模型因能力不足而较难解决的困难问题，因此重复失败的结构约束较少，累计提升较高。这一点也直接回答了为什么 verifier-retry 即便给定足够大的 $K$ 也无法替代思考模式，thinking 通过单次推理内部的多步搜索绕过状态追踪瓶颈，而 verifier-retry 只能给模型”再试一次“的机会，无法在参数层面给模型补充新的能力。这是两类缓解机制本质上的边界。

因此实测 pass@K 接近独立基线说明重试接近独立重采样，且反之也在暗示存在稳定结构瓶颈（详见 6.3 FailureHop），失败问题较集中；DeepSeek-V4-Flash 就属于这一情形，尽管其 pass@1 较高，但后续 pass@K 明显低于独立基线，说明首次失败后留下的问题集中、相关，存在稳定结构瓶颈，继续增加 retry 预算越容易提前饱和，不容易通过继续重试自然消除。同时这也解释了为什么 verifier-retry 对 DeepSeek-V4-Flash 有明显提升，但仍无法达到 thinking 模式的满分级表现；它提高的是有限预算内获得合法路径的概率，而不是直接消除模型内部的状态追踪瓶颈。下一节将进一步用 FailureHop 和 repeated same-hop failure 检验这一推测，分析这些剩余失败是否集中在少数状态转移位置上。
