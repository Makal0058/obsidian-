...
为进一步刻画 verifier-retry 的收益结构，将两个互补判据放在同一二维平面上。图 X 左侧首先比较了实测 pass@K 曲线与独立重试基线，独立重试零基线假设每次尝试都以 pass@1 估计出的固定概率 $p$ 独立通过 verifier，最多尝试 $K$ 次的预期通过率为 $1-(1-p)^K$，如果实测曲线贴近该基线则说明增加 retry 更接近于重复获得新的低概率机会；如果实测曲线明显低于该基线则说明后续 retry 并没有提供等价的独立新机会。图 X 右侧比较了 pass@K 相对独立重试零基线的偏离以及 FailureHop 在 hop 维度上的归一化熵。右图的横轴为平均独立基线缺口 $\bar{d}=\frac{1}{K_{\max}-1}\sum_{K=2}^{K_{\max}}\left(1-(1-p)^K-\mathrm{pass@}K\right)$，其中 $p$ 由 pass@1 估计，$\bar{d}$ 越大表示实测 pass@K 越低于独立重试基线，后续 retry 收益越不像独立重采样，剩余失败更可能受到相关错误或任务难度异质性的限制；纵轴为 FailureHop 的归一化熵，用于衡量失败跳数在路径中的分散程度，$H_{\mathrm{norm}}$ 越接近 1 说明失败越接近均匀分布；越接近 0，说明失败越集中在少数 hop。![[Pasted image 20260613153101.png]]图 X 中两个模型落在明显不同的位置，其中 Qwen Max 的实测 pass@K 与独立基线几乎重合，平均独立基线缺口接近 0，FailureHop 归一化熵为 0.927，最高单跳失败占比仅为 0.155。这一诊断解释了 verifier-retry 的饱和来源，Qwen 的 retry 收益形状接近独立重试基线，即低成功率下的重复采样，尽管单次通过率较低但每次 retry 仍提供一次重新尝试的低概率机会，从失败也可看出，较分散地出现在不同 hop（见 6.3 FailureHop）；相比下 DeepSeek-V4-Flash + retry 的实测 pass@K 从 K=2 开始明显低于独立基线，并且差距随 K 增大而扩大，说明饱和更多来自剩余失败的相关性或难度异质性，即首次失败后留下的样本不容易通过后续 retry 自然解决，后续剩余样本集中在更难修正的状态转移处，这些失败更可能反复暴露相似的状态转移错误（见 6.3 FailureHop）。从 DS-Flash 的平均独立基线缺口约为 0.217、FailureHop 归一化熵为 0.803、最高单跳失败占比为 0.370 且主要集中在 hop 2 也可看出剩余失败更集中，更偏离独立重试假设。![[Pasted image 20260613150247.png]]总而言之，上述协同分析表明 verifier-retry 的收益呈现出两层结构。第一层是 pass@K 随 retry 预算 $K$ 增加而上升，原因是 pass@K 衡量的是模型在最多 $K$ 次尝试中至少一次生成通过 verifier 路径的概率，因此随着 retry 预算 $K$ 增加 pass@K 通常上升，这一点主要来自 pass@K 的定义本身。然而这种提升并不是线性的，而会随着 $K$ 增加呈现边际收益递减，这便是第二层边际收益递减。第二层具体表现为两个维度，表面上 pass@1 决定了模型单次尝试的基础通过率，也决定了后续 retry 能够累积提升的基础空间，图 X 中 Qwen Max 属于这一情形，虽然 pass@1 较低，但其 pass@K 基本贴近独立重试零基线，说明后续 retry 更接近近似独立的低概率重采样，因此多次尝试仍能持续带来累计提升，从实测结果看，尽管 Qwen 的绝对水平远低于 DS-Flash（33.4% vs 70.6%），但相对提升幅度更大（$\Delta=25$），且这种低 p 下的独立基线下贴合独立基线带来的“更大相对提升”恰恰是本文下面想要强调的。

Qwen 单次通过率较低且失败近独立，其 pass@K 基本贴近独立重试基线，重复尝试的提升主要来自解决较容易问题，较少触及模型因能力不足而较难解决的困难问题，因此重复失败的结构约束较少，累计提升较高。这里独立重试零基线可以看作一种理想化情形，每次 retry 都像重新做一次题，错题之间没有强结构相关，在这种情况下的重复尝试主要解决偶发失误、容易出错但可恢复的问题；一旦某些样本本身超过了模型当前的状态追踪能力，模型并不会因为重复尝试就自然掌握正确转移。这也说明了独立重试基线为什么是合理的零模型，一个关键前提是 LLM 的 verifier-retry 不像人类做题，人类做错题后会记住错误积累经验，下一次重做时更新策略；而 LLM 调用 API 时参数不更新、上下文记忆仅限于本轮对话的反馈消息，模型在"难样本"上的内部状态追踪能力并不会随重试增强。因此每次 retry 在参数层面与第一次几乎完全等价——给定同样的输入和反馈格式,模型每次抽样面对的都是同一个固定的能力分布。这使得"独立重采样"在 LLM 上是合理的零模型，它对应 retry 的理论上界，即"若所有样本难度相同、且每次抽样独立"，pass@K 应满足 $1-(1-p)^K$。任何观察到的偏离都不来自“学习”而只能来自样本难度的异质性——存在一部分对该模型而言在"参数能力外"的样本，无论抽多少次都难以通过。这也把 pass@K 相对独立基线的偏离转化为对样本难度结构的诊断，若 pass@K 贴合基线（Qwen），说明在该模型视角下样本难度近似均匀——大多数样本都处在“低概率但有可能‘的同一区间，retry 类似在同一枚不公平硬币上反复抛掷，每次都有相同的小概率通过，但并不意味着 retry 在修复错误，而是意味着”样本对该模型而言基本均匀地难，多抛几次自然有几次走运通过“；若 pass@K 显著低于基线（DS-Flash），则说明该模型视角下样本难度强烈异质——简单样本被首次抽样吸收完，剩余样本对该模型的当前能力构成稳定瓶颈，每次重试对这些样本仍是低概率，且这个低概率不会因为”看了反馈“而提升，因为 LLM 无法跨轮积累经验。这一点也直接回答了为什么 verifier-retry 即便给定足够大的 $K$ 也无法替代思考模式，thinking 通过单次推理内部的多步搜索绕过状态追踪瓶颈，而 verifier-retry 只能给模型”再试一次“的机会，无法在参数层面给模型补充新的能力。这是两类缓解机制本质上的边界。
