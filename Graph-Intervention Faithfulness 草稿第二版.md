...
图 X 进一步展开了每个 retry attempt 下逐 hop 的 FailureHop 分布。该图显示，剩余失败并不是均匀分布在整条路径上，而呈现出明显的“首尾较硬、中段较软”结构。对于 DeepSeek-V4-Flash，第 2 跳形成稳定主峰，失败占比在五次 attempt 中始终维持在 0.33–0.39 之间，几乎不被 retry 削弱；第 3 跳在 attempt 1 时也较高，但随后从 0.21 下降到约 0.11，说明该位置的一部分失败可以被 retry 修正；第 12 跳则从 0.12 上升到 0.17–0.18，形成稳定的终端转移小峰。中间 hop 的失败占比整体较低，说明 DS-Flash 的剩余失败并不是随机散布，而是集中在少数关键状态转移位置。
...
相比之下，Qwen Max 的 FailureHop 分布更分散。它同样在早期 hop 有热点，但最高单 hop 占比从 attempt 1 的 0.27 下降到 attempt 5 的 0.16，第 2–4 跳的早期热区随 retry 被持续削平；与此同时，第 12 跳的占比从 0.04 上升到 0.14，中间 hop 的占比也更加均匀。这里的上升是剩余失败样本中的相对占比变化，并不意味着 retry 使终端错误的绝对数量增加。更合理的解释是，retry 优先削减了一部分可恢复失败，使剩余失败在相对比例上向更多 hop 分散。

因此，这张图将 §6.2 中的 pass@K 偏离定位到了路径中的具体位置。DeepSeek-V4-Flash 低于独立重试零基线，对应的是 hop 2 主峰长期稳定、难以被 retry 削弱；Qwen Max 贴近独立重试零基线，对应的是早期热点逐渐被削平、FailureHop 分布更分散。换言之，DS-Flash 更像“容易样本被前几次 retry 吸收后，剩余失败反复卡在少数状态转移位置”；Qwen 则更像“整体成功率低，但失败位置更分散，多次采样仍能持续捞回一部分样本”。
