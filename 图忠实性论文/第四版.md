...
随后，本文设置四个受控实验分析这两个问题。针对第一个问题，实验一比较完整任务与直接从末次竞争位置开始的局部任务，检验任务边界是否影响该位置的表现；实验二严格配对图实例，仅改变一个同源竞争者是否从该位置可达，检验竞争从无到有的影响。针对第二个问题，实验三给定一个已沿合法边进入无后继节点的固定前缀，并交叉操纵长度要求、逐边合法性声明和显式退出动作，分析规定长度与逐边合法性冲突时的失败处置；实验四在另一组配对图中，将一条中段黄金支持边原行原位反向，使候选答案由可达变为不可达，检验在未提供退出动作时，候选答案目标与逐边合法性冲突是否呈现相同的取舍方向。四个实验分别构成贡献二和贡献三的主要证据。
...
第二，本文发现，模型不能稳定判断合法候选能否支持剩余路径，其选择对候选的下游延展性与局部竞争均敏感。延展黄金候选的无分叉后级，使 GPT 与 Qwen 第一跳选中黄金后继的比例分别提高 40.40 和 37.63 个百分点，但该干预同时改变了规定跳数、终点距离和完整后缀，仅作为描述性证据。在严格配对条件下，将决策点出度由 $D=1$ 增至 $D=2$，仅使一个零出度同源候选变为局部可达，即使全部 18 个设置的成功率下降 5.75–25.25 个百分点；除该边相对于决策点的可达性外，其余图文本保持配对一致。

第三，本文发现，在未提供退出动作时，三个未启用推理模式的模型配置在两类任务目标—结构约束冲突中呈现相同的取舍方向：更常维持任务要求的输出目标，而牺牲必须回查当前输入图才能判定的逐边合法性。诊断实验中，12 跳路径—答案一致率为 79.88%–99.31%，而逐边微平均合法率仅为 65.58%–89.21%。在规定长度与逐边合法性冲突时，DeepSeek、GPT 与 Qwen 分别有 400/400、393/400 和 397/400 个响应包含图外转移；在候选答案与逐边合法性冲突时，维持候选答案而违反合法性的比例相对保持合法性而放弃候选答案分别高出 74.94、63.27 和 100.00 个百分点。长度冲突实验还表明，提供显式退出动作能够显著改变失败行为的分布。上述共同方向限于本文构造的两类冲突，尚不能区分任务目标属性与约束核验方式的影响。
# 2. 相关工作
## 2.1 忠实性的参照对象与测量方法
...
候选答案与逐边合法性的取舍。对候选答案—逐边合法性冲突实验，将缺失证据条件下的响应按结构分为：路径到达候选答案但包含图外转移、路径保持图上合法但未到达候选答案、两者均满足、两者均不满足或未输出路径。令 $E^{(j)}=1$ 表示模型在配对的完整证据条件下生成了到达候选答案的合法支持路径。主要结果量定义为 $\theta=\Pr(\text{到达候选答案但违反逐边合法性}\mid E=1)-\Pr(\text{保持逐边合法性但放弃候选答案}\mid E=1)$。$\theta>0$ 表示移除支持边后，模型更常维持候选答案；$\theta<0$ 表示更常维持逐边合法性。主分析限定于 $E=1$ 的配对实例，以排除模型在完整证据条件下本来就无法完成任务的情况；另在全部预定实例上直接计算两类响应比例之差，作为固定分母敏感性分析。完整分类与分母规则见附录 B.4。
# 4 实验设计
...
这一现象至少存在两种相容解释：第一，完整任务与直接从该位置开始的局部任务之间，是否存在由任务边界造成的性能差距？第二，在其他条件不变时，仅加入一个同源竞争者是否已经足以降低选择表现，继续增加候选又产生怎样的变化？§6.1通过任务边界干预与局部竞争干预分别检验。
## 5.3 输出内部合规与图结构合规发生解离

在同一 path_strict 契约下，路径—答案一致等输出内部合规与实例图结构合规明显解离。为区分单次转移错误与长路径上的累积效应，本文进一步报告 $\mathrm{EdgeLegal_{\mathrm{micro}}}$，即所有生成路径中合法相邻转移数占相邻转移总数的比例。12 跳时，DeepSeek、GPT 与 Qwen 的 TAC 分别为 99.31%、79.88% 和 99.19%，$\mathrm{EdgeLegal_{\mathrm{micro}}}$ 则分别为 89.21%、65.58% 和 74.76%。两组指标采用不同统计单位，但在三个模型上均呈现同一方向的差距。以 Qwen 为例，虽然 74.76% 的生成转移属于输入图中的合法边，但整条路径所有转移均合法的比例仅为 13.13%，同时满足起点、长度和逐边合法性的 $\mathrm{PathValid}$ 进一步降至 8.38%。这表明，路径—答案一致等输出内部合规不能替代对输入图的逐边验证。

上述结果仅表明两类合规表现发生解离。由于整路径合法性涉及随路径长度增加的多次边核验，本文不据此推断模型存在一般性的约束优先级；当两类约束被明确设为不可同时满足时的行为取舍，由 §6.2 的受控实验检验。

随着路径加深，这种结构差距进一步扩大。从 4 跳增至 12 跳，$\mathrm{PathValid}$ 分别由 DeepSeek 的 88.25% 降至 48.50%、GPT 的 71.69% 降至 8.06%、Qwen 的 68.12% 降至 8.38%。12 跳时，$\Delta_{\mathrm{illegal}}$ 在三个模型上分别达到 50.81%、71.81% 和 90.81%；GPT 的该指标在 10 跳达到 76.63% 后回落至 71.81%，主要由于其 $\mathrm{TAC}$ 本身下降。

进一步的首因分解显示，图外跳转逐渐成为长路径中的主要结构失败形式。起点与长度均正确但包含图外边的路径比例从 4 跳到 12 跳分别由 DeepSeek 的 0.62% 升至 26.94%、GPT 的 7.19% 升至 32.44%、Qwen 的 7.62% 升至 84.75%；完整结果见附录 C.3。

在 12 跳任务中，模型沿合法边进入零出度终止节点后，又明显分化为停止和继续生成图外边两种处置。两者占全部调用的比例分别为 DeepSeek 的 21.06% 与 29.50%、GPT 的 49.06% 与 22.00%、Qwen 的 4.75% 与 83.94%。由于诊断实验中模型是否进入该状态仍由上游生成决定，§6.2 固定模型已合法进入零出度节点的状态，并进一步操纵长度要求、逐边合法性声明和可用响应。
# 6 两类异常的受控检验
## 6.1 末次竞争位置的脆弱性

在起点固定为 $v_{k-2}$ 时，延展终端窗口使 GPT 的第一跳正确率由 55.66% 升至 96.00%，Qwen 由 61.88% 升至 99.50%；DeepSeek 保持在上界附近（99.03% 与 98.05%），暗示局部选择对候选的下游结构敏感。因此，贡献二末次竞争脆弱点受到候选下游结构的影响。由于该干预同时改变规定跳数、终点距离与完整后缀，本文不进一步区分其中哪一成分起主要作用。

$D$ 由 1 增至 2 使全部设置的 $p$ 下降，降幅 5.75–25.25 个百分点。$D=1$ 时，三个模型的 $p$ 为 99.25%–100.00%，表明在不存在同源竞争者时，模型几乎总能通过该位置。这一现象暗示模型对合法候选能否支持剩余路径的判断受局部竞争影响。
...
综合两项实验，末次竞争位置的脆弱性并不能由候选数量单独解释。完整任务与从 $v_{k-2}$ 开始的局部任务之间存在明显性能差距；同时，延展候选的下游结构后，GPT 与 Qwen 的首跳选择正确率大幅提高，表明该位置的局部选择还受到候选后续结构影响。进一步地，仅加入一个可达的同源零出度竞争者，就使联合成功率 $p$ 稳定下降 5.75–25.25 个百分点。因此，末次竞争失败同时表现出任务边界、候选下游结构与局部同源竞争三方面的敏感性。
## 6.2 任务目标与逐边合法性冲突下的取舍

本节在两类不可同时满足的约束设置中考察模型如何分配失败：一类令规定长度与逐边合法性冲突，另一类将规定长度替换为到达候选答案的任务目标。
### 6.2.1 规定长度与逐边合法性

固定终止前缀的 (2\times2\times2) 设计见附录 B.3。当规定长度与逐边合法性无法同时满足且不提供退出动作时，三个模型都主要继续产出。按逐边结构核验，DeepSeek、GPT 与 Qwen 分别有 400/400、393/400 和 397/400 个响应包含不属于输入图的相邻转移；DeepSeek 与 Qwen 通常生成图外节点，GPT 则大量使用空字符串维持规定的输出项数。当关闭长度要求且未额外声明逐边合法性时，三者分别有 397/400、400/400 和 400/400 个响应保留前缀并停止，说明冲突条件下的继续产出与长度要求有关。

进一步强调逐边合法性并未改变 DeepSeek 与 Qwen 的处置。开启合法性声明相对于关闭该声明，两者的非法续写率变化分别为 0.00 [0.00, 0.00] 和 0.00 [−1.25, 1.25] 个百分点（配对 cluster bootstrap 95% CI）。提供显式退出动作后，DeepSeek 的非法续写率由 100.00% 降至 0.00%，变化为 −100.00 [−100.00, −100.00] 个百分点；Qwen 由 99.00% 降至 7.00%，变化为 −92.00 [−94.75, −89.00] 个百分点。
...
因此，当规定长度与逐边合法性无法兼顾时，三个模型都主要维持输出长度形式而牺牲逐边一致性；重复强调合法性不足以改变这一取舍，而提供可执行的退出响应能够明显改变失败行为的分布。
### 6.2.2 候选答案与逐边合法性

为检验上述取舍是否局限于规定长度，本文进一步反向一条中段黄金边，使给定候选答案由存在合法支持路径变为不可达；模型仍被要求生成逐边合法且最终到达该候选答案的支持路径。该实验未提供显式退出动作，具体构造见附录 B.4。

完整证据条件下，DeepSeek、GPT与Qwen分别有399/400、392/400和392/400个响应同时到达候选答案并保持逐边合法。在这些完整证据条件下成功的配对实例中，支持边被移除后，三者分别有349/399、316/392和392/392个响应维持候选答案但违反逐边合法性；保持逐边合法但放弃候选答案的响应分别为50/399、68/392和0/392。GPT其余8例未落入上述两种取舍。

DeepSeek、GPT与Qwen的\(\theta\)分别为74.94、63.27和100.00个百分点，配对cluster bootstrap 95% CI分别为[67.92, 81.00]、[54.96, 70.69]和[100.00, 100.00]。以全部400个预定配对实例为固定分母时，三者的估计分别为75.00 [68.00, 81.00]、63.25 [55.75, 70.50]和100.00 [100.00, 100.00]个百分点，结论方向不变。因此，在未提供退出动作的输出契约下，三个未启用推理模式的模型配置都更常维持候选答案，而牺牲逐边合法性。
### 6.2.3 跨冲突设置的共同模式与解释边界

在未提供退出动作时，两类冲突在三个未启用推理模式的模型配置上呈现相同方向：规定长度与逐边合法性冲突时，模型主要维持输出长度形式；候选答案与逐边合法性冲突时，模型主要维持候选答案。两者共同表明，当任务目标与当前输入图中的逐边合法性无法同时满足时，这三个模型配置更常维持任务目标而牺牲逐边合法性。

这一结果不能解释为一般性的约束优先级。两项实验中，被维持的一侧既构成任务目标，也可由输出及简单任务参数直接核验，因此当前设计不能区分任务目标属性与核验方式的影响。该方向也未在一个补充的推理模式配置中复现：候选答案不可达时，该配置有39/40个响应拒绝继续支持给定答案，仅1/40生成图外转移。由于该比较同时改变了模型配置与推理模式，本文不将差异归因于推理模式。此外，§6.2.1表明提供退出动作能够改变失败分布，因此上述共同模式仅适用于未提供退出动作的输出契约。
# 7 结论
...
在本文构造的图任务中，诊断与受控实验进一步揭示了两类结构失败。第一，长路径失败集中暴露于最后一次同源竞争选择；该位置同时表现出任务边界差距、对候选下游结构的敏感性，以及严格配对识别出的局部同源竞争效应。第二，在任务目标与逐边合法性无法同时满足时，三个未启用推理模式的模型配置在两类冲突中呈现相同方向：未提供退出动作时，它们更常维持规定长度或给定候选答案，而牺牲当前输入图中的逐边合法性。显式退出动作能够改变失败行为的分布，但其使用方式和效果具有模型差异。

这些结果不构成一般性的约束优先级，也不解释模型内部如何形成上述取舍。它们表明，图推理中的结构忠实性必须独立于表面任务完成度进行检验。可靠的图推理评测不仅应检查答案与生成路径是否自洽，还应固定输出契约并逐边验证生成路径；当任务目标可能与当前图结构冲突时，还应显式定义可识别的退出响应，并分别评估非法续写、正确退出与错误退出。
# Limitations

第一，在要求恰好 $k$ 跳且路径唯一的图族中，最后一次竞争位置必然同时是黄金后继下游支持发生突变的位置；二者在该约束下不可分离。本文将该发现表述为该结构边界上的稳定脆弱性，不主张位置本身的独立效应。

第二，本文的主要统计推断仅覆盖三个未启用推理模式的模型配置。在一项40例的小样本补充实验中，另一个启用推理的配置在候选答案不可达且未提供退出动作时，有39/40个响应自行拒绝继续支持给定答案，仅1/40生成图外转移，方向与三个主要配置明显不同。由于该比较同时改变了模型检查点与推理模式，且样本量有限，本文不将这一差异归因于推理模式；其来源仍需在同一模型检查点下进行严格配对检验。

第三，本文在两类约束冲突中均观察到三个未启用推理模式的模型配置更常维持任务目标，而牺牲逐边合法性。但在当前设计中，规定长度与给定候选答案既是任务目标，也能够由输出及简单任务参数直接核验；相比之下，逐边合法性需要回查当前输入图。因此，本文不能区分这一取舍来自任务目标属性，还是来自不同约束的核验方式，也不据此主张一般性的约束类型优先级。
# Appendix A 图生成细节
...
| 模型       | hop | 有效图对 $N$ | $S_{\mathrm{last}}$ | $S_{\mathrm{serial}}$ | $S_{\mathrm{full}}$ | $\Delta_{\mathrm{serial}}$ | $\Delta_{\mathrm{full}}$ |   EAR | Schema符合率 |
| -------- | --: | -------: | ------------------: | --------------------: | ------------------: | -------------------------: | -----------------------: | ----: | --------- |
...
| 模型       | hop | $N$ | $S_{\mathrm{last}}$ | $S_{\mathrm{serial}}$ | $S_{\mathrm{full}}$ | $\Delta_{\mathrm{serial}}$ | $\Delta_{\mathrm{full}}$ |   EAR | Schema符合率 |
| -------- | --: | --: | ------------------: | --------------------: | ------------------: | -------------------------: | -----------------------: | ----: | --------- |
...
缺失证据条件下“两者兼得”均为 0，符合离线构造核验结果。三个模型均更常继续到达候选答案并产生图外转移，而不是保持逐边合法并放弃候选答案。
...
| ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| DeepSeek | 399 | 349/399 | 50/399 | 0/399 | 0/399 | 74.94 | [68.00, 81.00] |
