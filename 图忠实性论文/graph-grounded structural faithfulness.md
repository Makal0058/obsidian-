...
实验显示，三个未启用显式推理的模型在四跳链式图中，在仅要求输出答案配置下，模型的准确率达到 38.00%–71.00%，但在同时控制反事实图和序列化因素后均降至 0%。在十二跳分叉图中，模型生成路径的终点与其最终答案一致的比例达到 79.88%–99.31%，而逐边合法率仅为 65.58%–89.21%；移除关键支持边后，模型生成图外路径的风险进一步增加，相同趋势也出现在 MQuAKE 派生实验中。

在未提供显式退出动作的两类合成图受控冲突中，三个模型配置更常维持规定长度或候选答案，而牺牲逐边合法性；KQA Pro 的探索性实验中，部分模型的取舍方向尚不明确。
...
第二，本文发现，末次竞争位置的路径选择对局部同源竞争敏感。在严格配对条件下，仅增加一个局部可达的同源零出度竞争者，就使全部 18 个设置中通过该决策点的成功率下降 5.75–25.25 个百分点。此外，延展正确候选的后续路径，使GPT与Qwen在局部任务中的首跳选择正确率分别提高 40.40 和 37.63 个百分点；由于该干预同时改变了规定跳数、终点距离与后续路径结构，这一结果仅作为描述性证据。

第三，本文发现，在未提供显式退出动作的两类合成图受控冲突中，三个未启用推理模式的模型配置更常维持规定长度或候选答案，而牺牲逐边合法性。在MQuAKE派生的 200 例配对实验中，移除关键支持边使候选答案保持但路径非法的比例在DeepSeek、GPT与Qwen上分别提高 80.00、63.50 和 94.00 个百分点，表明相关现象并不局限于随机符号合成图。不过，KQA Pro的小规模预实验中，DeepSeek与Qwen的取舍差值区间跨越零，尚不能确定其取舍方向；提供显式退出动作也能改变冲突下的响应分布。因此，上述取舍模式具有明确的适用范围，本文不主张普遍的约束优先级。
# 2. 相关工作
## 2.1 忠实性的参照对象与测量方法
...
候选答案与逐边合法性的取舍。对候选答案—逐边合法性冲突实验，将缺失证据条件下的响应按结构分为：路径到达候选答案但未通过结构合法性检验、路径保持图上合法但未到达候选答案、两者均满足、两者均不满足或未输出路径。令 $E^{(j)}=1$ 表示模型在配对的完整证据条件下生成了到达候选答案的合法支持路径。主要结果量定义为 $\theta=\Pr(\text{到达候选答案但违反逐边合法性}\mid E=1)-\Pr(\text{保持逐边合法性但放弃候选答案}\mid E=1)$。$\theta>0$ 表示移除支持边后，模型更常维持候选答案；$\theta<0$ 表示更常维持逐边合法性。主分析限定于 $E=1$ 的配对实例，以排除模型在完整证据条件下本来就无法完成任务的情况；另在全部预定实例上直接计算两类响应比例之差，作为固定分母敏感性分析。完整分类与分母规则见附录 B.4。
# 4 实验设计
...
MQuAKE 和 KQA Pro 均使用 200 个四跳实例。迁移实验的主要比较是缺失证据相对于安慰剂条件的候选保持但路径非法比例，以及任意图外转移比例；$\theta$ 只描述缺失证据条件内部“维持候选答案”与“维持逐边合法性”的相对方向，不作为跨数据集必须同向的主要假设。
# 5 诊断实验结果
...
完整证据条件下，DeepSeek、GPT与Qwen分别有 399/400、392/400 和 392/400 个响应同时到达候选答案并保持逐边合法。在这些完整证据条件下成功的配对实例中，支持边被移除后，三者分别有 349/399、316/392 和 392/392 个响应维持候选答案但违反逐边合法性；保持逐边合法但放弃候选答案的响应分别为 50/399、68/392 和 0/392。GPT其余 8 例未落入上述两种取舍。

DeepSeek、GPT与Qwen的\(\theta\)分别为 74.94、63.27 和 100.00 个百分点，配对cluster bootstrap 95% CI分别为[67.92, 81.00]、[54.96, 70.69]和[100.00, 100.00]。以全部 400 个预定配对实例为固定分母时，三者的估计分别为 75.00 [68.00, 81.00]、63.25 [55.00, 70.75]和 100.00 [100.00, 100.00]个百分点，结论方向不变。因此，在这一合成图且未提供退出动作的输出契约下，三个未启用推理模式的模型配置都更常维持候选答案，而牺牲逐边合法性。
...
该方向也未在一个补充的推理模式配置中复现：候选答案不可达时，该配置有 39/40 个响应拒绝继续支持给定答案，仅 1/40 生成图外转移。由于该比较同时改变了模型配置与推理模式，本文不将差异归因于推理模式。此外，§6.2.1 表明提供退出动作能够改变失败分布，§6.3 进一步表明，在KQA Pro的小规模预实验中，部分模型的取舍方向尚不明确，因此上述共同模式只描述本文的合成图、三个主要模型配置和未提供退出动作的输出契约。

## 6.3 基准数据派生实验中的迁移与异质性

MQuAKE 的 200 例四跳配对实验复现了关键证据移除后的图外生成增长。以全部预定实例为固定分母，缺失支持边相对于安慰剂处理使候选答案保持但路径非法的比例在 DeepSeek、GPT 与 Qwen 上分别提高 80.00、63.50 和 94.00 个百分点，任意图外转移比例分别提高 83.50、64.00 和 95.00 个百分点。三个模型在缺失证据条件内部也都更常维持候选答案而非逐边合法性，但该结果只支持 MQuAKE 派生设置中的迁移。

KQA Pro 的 30 例预实验同样观察到缺失证据相对于安慰剂处理增加图外生成，但 DeepSeek 与 Qwen 的 \(\theta\) 区间跨越 0，尚不能确定其取舍方向。由于样本较小，本文将该实验作为探索性外部效度证据，而不用于估计普遍的模型排序。

因此，跨合成图与基准派生图更稳定的结论是：移除关键支持证据会提高候选保持型图外生成的风险；现有结果尚不足以确立跨任务、跨模型普遍一致的取舍方向。该结果支持逐边符号核验的必要性，同时反对把合成冲突中的同向结果解释为普遍的约束优先级。
# 7 结论
...
诊断与受控实验进一步揭示了两类结构失败。第一，长路径失败集中暴露于最后一次同源竞争选择；该位置同时表现出任务边界差距、对候选下游结构的敏感性，以及严格配对识别出的局部同源竞争效应。第二，在合成图中，三个未启用推理模式的模型配置在两类任务目标—合法性冲突中都更常维持规定长度或给定候选答案；MQuAKE 派生实验复现了关键证据移除后候选保持型图外生成的增长，而 KQA Pro 预实验中部分模型的取舍方向尚不明确。显式退出动作能够改变失败行为的分布，但其使用方式和效果同样具有模型差异。
...
第三，本文在两类合成约束冲突中均观察到三个未启用推理模式的模型配置更常维持任务目标，而牺牲逐边合法性。但在当前设计中，规定长度与给定候选答案既是任务目标，也能够由输出及简单任务参数直接核验；相比之下，逐边合法性需要回查当前输入图。MQuAKE 的迁移结果支持同向的候选保持模式，但 KQA Pro 小规模预实验中部分模型的取舍方向尚不明确，现有证据不足以确立普遍的约束优先级。因此，本文不能区分合成图中的取舍来自任务目标属性还是不同约束的核验方式，也不据此主张一般性的约束类型优先级。
# Appendix A 图生成细节
