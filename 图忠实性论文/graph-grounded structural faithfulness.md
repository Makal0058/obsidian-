...
第三，本文发现，在未提供显式退出动作的两类合成图受控冲突中，三个未启用推理模式的模型配置更常维持规定长度或候选答案，而牺牲逐边合法性。在 MQuAKE 派生的 200 例配对实验中，移除关键支持边使候选答案保持但路径非法的比例在 DeepSeek、GPT 与 Qwen 上分别提高 80.00、63.50 和 94.00 个百分点，表明相关现象并不局限于随机符号合成图。不过，提供显式退出动作能够改变冲突下的响应分布。因此，上述取舍模式具有明确的适用范围，本文不主张普遍的约束优先级。
# 2. 相关工作
## 2.1 忠实性的参照对象与测量方法
...
MQuAKE 使用 200 个四跳实例；KQA Pro 已按新版方案构造并冻结 200 个四跳实例，模型结果待补。MQuAKE 额外要求输出恰好四跳，而 KQA Pro 的四跳仅指完整证据中的支持路径长度，不额外限制输出跳数。MQuAKE 主要比较缺失证据相对于安慰剂条件的“路径到达候选且含图外转移”比例差及任意图外转移比例差；KQA 分别报告候选保持但路径检验失败与任意图外转移的比例差。为区分逐边合法性与长度合规，MQuAKE 另对已有输出进行不含长度要求的补充评分，保留原四跳评分，并以 $\theta_{\mathrm{edge}}$ 描述补充评分下的响应取舍。完整构造、prompt 与评分规则见附录 B.5。
# 5 诊断实验结果
...
本章方括号内为配对 bootstrap 估计的 95% 置信区间，重采样单位见相应实验附录。
...
完整证据条件下，DeepSeek、GPT 与 Qwen 分别有 399/400、392/400 和 392/400 个响应同时到达候选答案并保持逐边合法。在这些完整证据条件下成功的配对实例中，支持边被移除后，三者分别有 349/399、316/392 和 392/392 个响应维持候选答案但违反逐边合法性；保持逐边合法但放弃候选答案的响应分别为 50/399、68/392 和 0/392。GPT 其余 8 例未落入上述两种取舍。
...
不含长度要求的补充评分允许将从正确起点沿合法边前进后停止记为 graph_only，但三个模型在缺失证据条件下均未出现该类响应。以完整证据下结构成功的实例为条件，$\theta_{\mathrm{edge}}$ 分别为 96.39 [93.37, 98.80]、83.78 [77.70, 89.86] 和 98.94 [97.34, 100.00] 个百分点。原四跳评分下 graph_only 在缺失图中不可实现，而补充评分下其计数为零是观察结果。上述结果描述原提示词下候选保持与逐边合法性两类响应的分布，不直接识别模型内部偏好；完整分类及固定分母结果见附录 D.5。

> 待补实验结果：本节 KQA Pro 采用附录 B.5.2 的 200 例四跳冻结任务。完成模型运行后，补入缺失证据相对于安慰剂的两项比例差、$\theta$ 及其置信区间，并据此更新跨设置总结、摘要、贡献与结论。旧 30 例两跳预实验不作为该新版实验的结果。

目前，合成图与 MQuAKE 派生实验共同支持关键证据移除会提高候选保持型图外生成风险。KQA Pro 四跳结果尚待补齐，本文不据此预判其取舍方向，也不将现有同向结果推广为普遍的约束优先级。
# 7 结论
...
诊断与受控实验进一步揭示了两类结构失败。第一，长路径失败集中暴露于最后一次同源竞争选择；该位置同时表现出任务边界差距、对候选下游结构的敏感性，以及严格配对识别出的局部同源竞争效应。第二，在合成图中，三个未启用推理模式的模型配置在两类任务目标—合法性冲突中都更常维持规定长度或给定候选答案；MQuAKE 派生实验复现了关键证据移除后候选保持型图外生成的增长。显式退出动作能够改变失败行为的分布，但其使用方式和效果同样具有模型差异。
...
第三，本文在两类合成约束冲突中均观察到三个未启用推理模式的模型配置更常维持任务目标，而牺牲逐边合法性。但在当前设计中，规定长度与给定候选答案既是任务目标，也能够由输出及简单任务参数直接核验；相比之下，逐边合法性需要回查当前输入图。MQuAKE 的迁移结果支持同向的候选保持模式，但现有证据不足以确立普遍的约束优先级。因此，本文不能区分合成图中的取舍来自任务目标属性还是不同约束的核验方式，也不据此主张一般性的约束类型优先级。
# Appendix A 图生成细节
...
MQuAKE 的主要迁移对比为缺失证据减安慰剂条件的“路径到达候选且至少存在一次可核验图外转移”比例差，以及任意图外转移比例差，以全部 200 个预定实例为固定分母。该交集独立计算，不由 candidate_only 推断。补充评分的 $\theta_{\mathrm{edge}}$ 定义为缺失证据条件内 candidate_only 与 graph_only 的比例之差，主条件分析限定于配对完整证据下生成到达候选的合法路径的实例，并报告固定分母敏感性分析；无路径及其他失败保留在相应分母中。答案字段、长度和完整输出契约合规单独报告。MQuAKE 原有的 clean 辅助评分仍保留在历史报告中，但同样含四跳要求，不用于本文的行为取舍解释。KQA 当前分析器分别报告 candidate_only 比例差、任意图外转移比例差及不含长度要求的 $\theta$；其 candidate_only 同样不能在未经交集核验时直接称为候选保持型图外生成。

95% 置信区间采用实例级配对 bootstrap，同一实例的三个条件共同重采样：MQuAKE 原结构评分及补充评分使用 5,000 次重采样、种子 20260813；KQA 使用 2,000 次重采样、种子 20260908。此次 MQuAKE 补充分析核对了三个模型各 600 条成功调用记录的任务身份与指纹；缺失或重复记录会阻止正式分析。KQA 正式分析同样要求冻结任务结果齐全，缺失调用时仅允许输出明确标记的临时分析。区间反映该构造样本在实例重采样下的不确定性，不声称覆盖知识库总体或已消除跨实例实体复用造成的依赖。
