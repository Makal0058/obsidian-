...
图 1 展示图任务、答案级与路径级指标，以及它们与四个研究问题的对应关系。答案级控制检查成功是否依赖呈现方式；路径级核验检查响应是否得到当前图支持；后续配对实验用于分析已定位的异常。

![GIF 方法总览](attachments/GIF_方法总览_中文.png)

图 1：GIF 的图任务、双层诊断指标与四个研究问题。反事实图对及输出契约用于区分答案层的表面成功与路径层的结构遵循；后续实验分别检验局部竞争、任务要求冲突以及真实来源证据图中的关键边依赖。图中示意 answer_only 与 path_strict；path_basic 作为独立输出契约在实验中另行报告。
...
表 1 将四跳链式图、六跳链式图、六跳分支图和十二跳分支图横向串联。两个链式图在 answer_only 下具有非平凡的末位条件成功率，但跨序列化与诱饵控制后均收缩至零；六跳分支图在 path_strict 下同时展示答案级配对成功与路径级合法率，并保持两类分母的区别。EAR 表明模型经常选中列表末尾、但需要多走一步才能到达的诱饵节点。

表 1：跨跳数的答案级与路径级诊断。四跳链式图展示表面反事实成功经控制后的收缩；六跳链式与分支图衔接两层现象；十二跳分支图展示路径—答案一致仍不足以保证完整路径合法。单位为 %，各面板的图结构和输出契约不同，数值不可直接比较。四跳及六跳分支图的答案指标每模型使用 200 个有效图对；六跳链式图 DeepSeek、GPT、Qwen 分别使用 198、199、200 个有效图对；TAC、PathValid 和 $\Delta_{\mathrm{illegal}}$ 每模型使用 1,600 个响应，逐边合法率使用生成转移分母。

![跨跳数诊断表](attachments/表3_跨跳数诊断.png)
...
表 1 右侧十二跳分支图的结果表明，即使提示已逐项列出结构要求，高路径—答案一致率仍可伴随低完整路径合法率。Qwen 的 TAC 为 99.19%，但 PathValid 仅为 8.38%；DeepSeek 的对应指标为 99.31% 与 48.50%。模型可以在输出内部保持一致，同时违反输入图约束。
...
表 2：局部任务的终止窗口对照。准确率为 %；配对变化为 pp。配对变化来自可配对样本及原稿规定的聚合方式，不能由展示的单元格均值直接相减重建。
...
表 3：由 $D=1$ 增至 $D=2$ 的联合通过率变化，单位为 pp。负值表示引入竞争后表现下降。
...
实验 3 是任务要求与退出选项的配对消融实验。400 个十二跳分支实例均提供一条合法的十一跳前缀，且前缀终点无合法后继；保留此前缀就无法合法完成剩余一跳。表 4 列出三因素的八种条件，每个模型计划 3,200 次调用；完整结果见附录 D。

表 4：长度要求、逐边合法性声明与退出选项的配对消融设计。在图结构、边序和终止前缀固定时，分别开关 L、V、X，构成八种实验条件，用于区分任务要求与退出动作对不可满足状态下模型响应的影响。

| 长度要求 L | 合法性声明 V | 退出选项 X | 主要比较目的 |
|:--|:--|:--|:--|
| 关闭 | 关闭 | 无 / 有 | 观察不增加额外要求时的响应 |
