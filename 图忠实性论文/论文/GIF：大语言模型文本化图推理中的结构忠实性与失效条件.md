...
**表 3：六跳分支图在 path_strict 下的答案级与路径级诊断**。跨呈现条件的联合检验使答案成功率明显收缩，而较高的路径—答案一致率与较低的完整路径合法率并存，表明单一呈现条件下的答案成功与输出自洽均不足以证明结构遵循。

| 模型       | $S_{\mathrm{last}}$ | $S_{\mathrm{serial}}$ | $S_{\mathrm{full}}$ | TAC    | PathValid | $\Delta_{\mathrm{illegal}}$ |
| -------- | ------------------- | --------------------- | ------------------- | ------ | --------- | --------------------------- |
| DeepSeek | 35.50               | 20.50                 | **14.00**           | 99.69  | **78.56** | 21.12                       |
| GPT      | 14.50               | 0.00                  | **0.00**            | 94.31  | **43.69** | 50.62                       |
| Qwen     | 21.00               | 6.00                  | **2.50**            | 100.00 | **54.19** | 45.81                       |
...
**路径—答案一致不保证路径合法。** 表 4 显示，即使提示已逐项列出结构要求，高路径—答案一致率仍可伴随很低的完整路径合法率，说明输出内部自洽并不能保证生成路径受到输入图支持。
...
**固定终止状态：长度、合法性与退出动作。** 实验 3 在 400 个十二跳分支实例中固定一条合法的十一跳前缀，其终点无合法后继，因此无法保留此前缀并合法完成剩余一跳。在图结构、边序和前缀固定的条件下，交叉操纵长度要求 $L$、显式逐边合法性要求 $V$ 与显式退出动作 $X$，形成 $2\times2\times2$ 配对设计，每个模型计划 3,200 次调用。
