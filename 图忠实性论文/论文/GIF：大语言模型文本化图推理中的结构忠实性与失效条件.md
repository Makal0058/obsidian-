...
文本化图推理要求大语言模型生成的答案与路径得到输入图支持。然而，正确答案可能依赖文本位置线索，路径与答案自洽也不保证路径合法，仅凭这些表面成功信号可能高估模型对图结构的遵循程度。为此，我们提出 Graph-Intervention Faithfulness（GIF）诊断框架，结合反事实图对、序列化控制与逐边验证，分别检验答案的跨呈现条件稳定性与生成路径的结构合法性。实验表明，单一呈现条件下的反事实答案成功不足以保证跨条件稳定正确；在十二跳分支图的 path_strict 设置下，三个主要模型配置的路径—答案一致率达到 79.88%–99.31%，但完整路径合法率仅为 8.06%–48.50%。严格配对干预进一步发现，引入一个同源零出度竞争后继，使全部 18 个设置中通过末次竞争节点的联合成功率下降 5.75–25.25 个百分点。在无显式退出的两类合成冲突中，三个主要配置更常维持指定列表长度或候选终点，却未同时满足结构要求；固定终止状态实验表明，提供退出动作会改变响应分布，但具体表现存在模型差异。基于 MQuAKE 与 KQA Pro 构造的真实来源证据图也显示，相比安慰剂处理，破坏关键支持边会增加图外转移风险。上述结果表明，答案正确、输出自洽与结构遵循需要分别检验；评测应明确输出契约、逐边核验证据，并区分非法续写、提前停止与显式退出。
## 1 引言
...
在完整证据下成功的配对实例中，MQuAKE 的三个模型均未观察到 graph_only 响应，且 $\theta_{\mathrm{edge}}$ 的置信区间均高于零。KQA Pro 中，GPT 与 Qwen 的 $\theta$ 区间高于零，而 DeepSeek 为 11.98 [−2.99, 26.95] pp，相对取舍方向尚不明确。完整条件分母、估计值及置信区间见附录 D.5–D.6。
