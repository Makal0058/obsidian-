...
对第 $j$ 个反事实图对，若模型在序列化条件 $\alpha$ 下于两张图上均输出各自的黄金答案，记 $C_{\alpha}^{(j)}=1$，否则为 0。记模型生成的答案为 $\hat y$，生成路径为 $\hat p=(\hat v_0,\hat v_1,\ldots,\hat v_m)$，其中 m 为实际生成的跳数，$\mathrm{EAR}^{(j)}=\frac{1}{2}\sum_{r=1}^{2}\mathbb 1[\hat y_{r,\mathrm{last\_decoy}}^{(j)}=z_r^{(j)}]$，表示 last_decoy 条件下模型将末位诱饵节点报告为答案。诊断实验使用的双层指标如下。
...
令生成路径为 $\hat p=(\hat v\_0,\ldots,\hat v\_m)$，生成答案为 $\hat y$，其中 $m$ 是实际跳数。路径—答案一致性为
$\mathrm{TAC}$，完整路径合法性为 $\mathrm{PathValid}$，二者的分离由下式记录 $\Delta\_{\mathrm{illegal}}=\mathrm{TAC}(1-\mathrm{PathValid})$。
...
| 实验对象   | 指标 / 变量                                 | 定义与含义                                          |
| ------ | --------------------------------------- | ---------------------------------------------- |
| 末次竞争选择 | $q_{\mathrm{reach}}$                    | 正确抵达该位置的概率                                     |
| 末次竞争选择 | $p_{\mathrm{cond}}$                     | 正确抵达后选择黄金后继的概率                                 |
| 末次竞争选择 | $p=q_{\mathrm{reach}}p_{\mathrm{cond}}$ | 正确抵达且选择黄金后继的联合成功率                              |
| 无合法后继  | $Y_{\mathrm{term}}\in\{R,S,I,O\}$       | 显式退出、保留前缀后停止、非法续写、其他不合规响应                      |
| 候选答案冲突 | $\theta$                                | 完整证据下成功的配对实例中，candidate_only 与 graph_only 的概率差 |
