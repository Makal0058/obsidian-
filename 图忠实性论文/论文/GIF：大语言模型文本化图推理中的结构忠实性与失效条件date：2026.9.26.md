...
GIF 以当前输入图为外部可验证参照，从答案层和路径层检验结构遵循。答案层通过反事实图对与序列化控制，检查正确响应能否跨呈现条件保持稳定；路径层将路径—答案一致性与路径合法性分开核验，识别输出自洽但缺乏图结构支持的响应。随后，逐跳诊断与配对实验进一步分析失败发生的位置及其条件。

**图 1：GIF 诊断与受控分析框架概览**。从答案级与路径级诊断表面成功和结构遵循之间的差距，并通过受控干预分析失效因素，在真实来源证据图上进一步验证。
![GIF 方法总览](attachments/GIF_方法总览_中文.png)

本节依次介绍图任务与输出契约、诊断实验指标及受控实验结果变量，整体流程见图 1，实例生成规则、prompt 原文及诊断指标的详细说明见附录 A。
## 3.1 图任务设定
...
![表3：答案级与路径级诊断](attachments/表3_跨跳数诊断.png)

注：数值单位为 %。链式图答案级指标以有效反事实图对为分母：四跳每模型 200 对；六跳 DeepSeek、GPT、Qwen 分别为 198、199、200 对。分支图路径级指标每个深度、每个模型均以 1,600 条响应为分母。六跳链式图与六跳分支图的拓扑和输出契约不同，两组结果不能直接比较。
