...
同时分叉图中当前节点 $v_i$ 有多个后继节点，模型必须在多个后继节点里选中正确节点，理性化下近似为成功概率近似为 $\Pr[\text{局部状态追踪成功概率}]\approx \frac{1}{|\mathcal{N}^{+}(v_i)|}$，状态追踪难度明显增大。上述分析说明，预期收益降低导致模型策略转移，状态追踪难度增大导致失败风险结构变化，这两个机制层次使失败模式也应相应转移，从链式图的位置捷径转移为分叉图的状态追踪失败，通常呈现三类递进形式：第一，模型在某个分叉点选择一条合法但错误的候选边，偏离黄金路径走向终止分支；第二，偏离黄金路径后，由于剩余 $hop$ 数不满足任务要求，或当前分支无法抵达黄金答案，模型进一步生成输入图中不存在的非法边；第三，模型围绕这条非法路径声明一个与路径末节点一致的最终答案。后两类失败正是 $\Delta_{\mathrm{illegal}}$ 的诊断目标，也是从链式图到分叉图的设计基础。分叉图以 branching-12hop 作为主要困难设置，用于诊断多状态更新下的非法路径和状态追踪失败，同时以 branching-4hop 作为位置捷径与非法路径的分离探针，朴素策略的回报折扣对不同模型可能产生不同后果：当任务中包含足够多的分叉结构时，固定位置策略不再具有稳定的泛化优势。能力较强模型的预测行为更可能转向真实状态追踪（错误模式从答案层 GFI 迁移到路径层 $\Delta_{\text{illegal}}​$）；能力较弱的模型由于本身状态追踪能力有限，更换状态追踪策略后的预期回报可能低于折扣的位置策略，则此时模型的最优选择是继续依赖位置捷径（GFI 仍然高）。branching-4hop 作为模型策略选择的探针，通过与 chain-4hop 保持相同 $hop$ ，解耦模型选择"位置捷径失效后转向状态追踪"还是"本身状态最终能力不足，导致位置捷径依然存在"。若模型在 branching-4hop 上 GFI 显著下降，则应理解为位置策略被放弃；若保持较高 GFI 则说明位置策略被沿用。
